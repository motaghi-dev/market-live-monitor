...
## GitHub checklist (before you push)
...
If a key was already shared, rotate it.
...
If you want a minimal one, start with:
```txt
faster-whisper
requests
```
...
### “No MP3 files are created”
- Make sure `ffmpeg` and `yt-dlp` work in the same terminal:
  ```bat
  ffmpeg -version
  yt-dlp --version
  ```
- Check that `cookies.txt` exists in the repo root.
- Look at the logs in `./logs/` (yt-dlp and ffmpeg logs are written there).

### ffmpeg says: “Invalid data found when processing input (pipe:0)”
This usually means yt-dlp stopped producing valid output (or produced nothing).  
Check `logs/ytdlp.log` first. Common reasons:
- cookies expired / invalid
- YouTube blocked the request
- the livestream URL changed

### faster-whisper crashes with “Windows fatal exception: access violation”
This is usually a Python/package binary mismatch on Windows.
- Use Python **3.11**
- Make sure you are running the venv Python:
  ```bat
  .\.venv\Scripts\python.exe -c "import sys; print(sys.executable)"
  ```
- If it still happens, try:
  - `MODEL_SIZE = "base"` (smaller)
...
I received these repo-root files:
- AudioSaving.py
- AudioSaving_overlap.py
- transcribe_faster_whisper.py
- summarize_openrouter.py
- tag_entities_rules.py
- db.py
- init_db.py
- stocks_universe.csv

If you also have any of these, consider adding them to the repo:
- `requirements.txt`
- `.gitignore`
- a sample `.env.example` (no secrets inside)
